...
RefPtr<Pad> blockpad;
RefPtr<Element> conv_before, conv_after, curr_effect;
...
PadProbeReturn pad_probe_cb(const RefPtr<Pad>& pad, const PadProbeInfo& info) {
    ...
    srcpad->add_probe(PAD_PROBE_TYPE_BLOCK
            | PAD_PROBE_TYPE_EVENT_DOWNSTREAM, sigc::ptr_fun(event_probe_cb));
    RefPtr<Pad> sinkpad = curr_effect->get_static_pad("sink");
    sinkpad->send_event(EventEos::create());
    return PAD_PROBE_OK;
}

bool on_bus_message(const Glib::RefPtr<Bus>&, const Glib::RefPtr<Message>& message) {
