...
int main(int argc, char** argv)
{
    ...
    Glib::RefPtr<Gst::Element> source = Gst::ElementFactory::create_element("filesrc"),
            ...
            goom = Gst::ElementFactory::create_element("goom2k1"),
    ...
    // Link tee with queues
    Glib::RefPtr<Gst::PadTemplate> tee_src_tpl = tee->get_pad_template("src_%u");
    if (tee->request_pad(tee_src_tpl)->link(audio_queue->get_static_pad("sink"))
            != Gst::PAD_LINK_OK) {
        std::cerr << "Cannot link tee pad with audio queue" << '\n';
        return 1;
    }
    if (tee->request_pad(tee_src_tpl)->link(video_queue->get_static_pad("sink"))
            != Gst::PAD_LINK_OK) {
        std::cerr << "Cannot link tee pad with video queue" << '\n';
        return 1;
    }
    ...
    std::cout << "Setting pieline to PLAYING." << '\n';
