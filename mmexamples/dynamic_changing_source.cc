...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        std::cout << "Usage: " << argv[0] << " <avi output filename>" << '\n';
        return 1;
    }
    ...
    if (!pipeline) {
        std::cout << "Cannot create pipeline element." << '\n';
        return 1;
    }

    auto queue1 = ElementFactory::create_element("queue");
    auto queue2 = ElementFactory::create_element("queue");
    auto tee = ElementFactory::create_element("tee");
    auto autosink = ElementFactory::create_element("autovideosink");
    auto sink = ElementFactory::create_element("filesink");
    auto muxer = ElementFactory::create_element("avimux");
    ...
    try {
        ...
        std::cout << "Exception occured during preparing pipeline. Message: "
                << ex.what() << '\n';
        return 1;
    }
